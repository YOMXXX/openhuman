...
fn migrate_legacy_embeddings_to_sidecar(conn: &Connection, config: &Config) -> Result<()> {
    ...
    if has_uncovered {
        let backfill_job = crate::openhuman::memory_tree::jobs::types::NewJob::reembed_backfill(
            &crate::openhuman::memory_tree::jobs::types::ReembedBackfillPayload {
                signature: sig.clone(),
            },
        )?;
        crate::openhuman::memory_tree::jobs::enqueue_tx(&tx, &backfill_job)?;
        crate::openhuman::memory_tree::jobs::set_backfill_in_progress(true);
    }
