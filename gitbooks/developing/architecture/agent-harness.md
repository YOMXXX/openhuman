...
* **Action budget policy** - `SecurityPolicy` enforces `config.autonomy.max_actions_per_hour` for side-effecting tool operations. Users can tune it in Settings -> Advanced -> Action Budget, or operators can override it with `OPENHUMAN_MAX_ACTIONS_PER_HOUR`.
